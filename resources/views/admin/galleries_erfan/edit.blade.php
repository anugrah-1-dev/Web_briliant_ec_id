...
@section('content_header')
    <h1><i class="fas fa-edit mr-2 text-warning"></i> Edit Galeri Erfan: {{ $gallery->title }}</h1>
    @if ($errors->any())
...
@section('content')
    <div class="card card-outline card-warning">
        <div class="card-body">
            <form action="{{ route('admin.galleries-erfan.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
    ...
    <div class="card mt-4 card-outline card-warning">
                        ...
                        <div class="card shadow-sm">
                            ...
                            @else
                                <img src="{{ asset('storage/' . $image->image_path) }}"
                                    class="card-img-top" style="height:140px; object-fit:cover;" alt="Foto">
                            @endif
                            <div class="card-body p-2 text-center">
                                ...
                                <button class="btn btn-sm btn-danger btn-block btn-delete-image"
                                    data-id="{{ $image->id }}"
                                    data-url="{{ route('admin.galleries-erfan.images.destroy', $image->id) }}">
            ...
            <div class="modal fade" id="editVideoModal{{ $image->id }}" tabindex="-1" role="dialog">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <form action="{{ route('admin.galleries-erfan.images.update', $image->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
            ...
            btn.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        document.getElementById('delete-image-form').setAttribute('action', url);
